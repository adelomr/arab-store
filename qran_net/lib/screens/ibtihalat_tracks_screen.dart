...
import '../models/ibtihalat_artist.dart';
...
class IbtihalatTracksScreen extends StatefulWidget {
  final IbtihalatArtist artist;

  const IbtihalatTracksScreen({super.key, required this.artist});

  @override
  State<IbtihalatTracksScreen> createState() => _IbtihalatTracksScreenState();
}

class _IbtihalatTracksScreenState extends State<IbtihalatTracksScreen> {
  final ApiService _apiService = ApiService();
  List<Ibtihalat> _tracks = [];
  List<Ibtihalat> _filteredTracks = [];
  ...
  Future<void> _loadTracks() async {
    final tracks = await _apiService.fetchIbtihalatTracks(widget.artist.id);
    if (mounted) {
      setState(() {
        _tracks = tracks;
  ...
  Future<void> _downloadTrack(Ibtihalat track) async {
    final fileName = "ibtihalat_${track.title.hashCode}.mp3";
    ...
    try {
      await _downloadService.downloadFile(
        ...
        title: track.title,
        category: DownloadCategory.speech, // Reusing speech category for downloads
  ...
  void _playAndNavigate(BuildContext context, Ibtihalat track) {
    final audioProvider = Provider.of<AudioProvider>(context, listen: false);
    audioProvider.playIbtihalatArtist(
      track, 
            ...
            IconButton(
              ...
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => FavoritesScreen(typeFilter: 'ibtihalat'),
                  ),
                );
              },
            ),
